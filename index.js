document.addEventListener('DOMContentLoaded', () => {
    ...
    async function registerSupplier() {
        ...
        try {
            ...
            if (response.ok) {
                messageDiv.textContent = `הרישום הצליח! שם המשתמש שלך הוא: ${username} (סיסמה: ${password} - יש לשמור!).`;
                messageDiv.className = 'success';
                if (data.token) {
                    saveToken(data.token);
                }
                saveUsername(username);
                window.location.href = `supplier_home.html?username=${username}`;
            } else {
